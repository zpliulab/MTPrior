...
from CLassificationAnalysis import RClassification, AdaBoostClassification,NaiveBayesClassification, XGBoostClassification
import os
...
hid_units = [32] #4>64 8>128 16>256 #32>512
...
def load_data_dblp():
    ...
    in_lml = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmlnc.txt')
    in_lmil = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmilnc.txt')
    in_lmimmil = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmimmilnc.txt')

    in_lml2 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmlnc2.txt')
    in_lmil2 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmilnc2.txt')
    in_lmimmil2 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmimmilnc2.txt')

    in_lml3 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmlnc3.txt')
    in_lmil3 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmilnc3.txt')
    in_lmimmil3 = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_lncmimmilnc3.txt')

    in_feature = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_fea_2.txt').astype(float)
    in_label = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_label_2.txt')
    in_train = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_train.idx.txt')
    in_val = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_val.idx.txt')
    in_test = np.loadtxt('/home/fatemeh/MTPrior/data/HCC/lncRNA-import/PN_test.idx.txt')

    truelabels, truefeatures = in_label, in_feature.astype(float)
    N = truefeatures.shape[0]

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N)]

    #rownetworks = [in_lml - np.eye(N), in_lmil - np.eye(N), in_lmimmil - np.eye(N),
     #              in_lml2 - np.eye(N), in_lmil2 - np.eye(N) ,  in_lml3 - np.eye(N)]

    #rownetworks = [in_lmil - np.eye(N), in_lmil2 - np.eye(N), in_lmil3 - np.eye(N)]

    #rownetworks = [in_lml - np.eye(N), in_lml2 - np.eye(N), in_lml3 - np.eye(N)]
    rownetworks = [in_lml - np.eye(N), in_lml2 - np.eye(N)]

                   #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
        #           in_lml2 - np.eye(N), in_lmil2 - np.eye(N), in_lmimmil2 - np.eye(N),
            #       in_lml3 - np.eye(N), in_lmil3 - np.eye(N)] #58

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
      #           in_lmil2 - np.eye(N), in_lmimmil2 - np.eye(N)] #0.90

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
     #              in_lmil2 - np.eye(N)]

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
        #                   in_lmil2 - np.eye(N), in_lmimmil2 - np.eye(N), in_lmilml - np.eye(N)] #0.87

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
        #           in_lmimmil2 - np.eye(N), in_lmil2 - np.eye(N),
          #          in_lmil3 - np.eye(N), in_lmimmil3 - np.eye(N)] #87

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
    #               in_lmimmil2 - np.eye(N), in_lmil2 - np.eye(N),
     #              in_lmilml - np.eye(N)]  #94 #Emb_lnc1.txt

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
         #         in_lmimmil2 - np.eye(N), in_lmil2 - np.eye(N),
           #        in_lmimmil3 - np.eye(N)]  #80

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
       #            in_lml2 - np.eye(N), in_lmimmil2 - np.eye(N), in_lmil2 - np.eye(N),
        #           in_lml3 - np.eye(N), in_lmil3 - np.eye(N), in_lmimmil3 - np.eye(N),
          #         in_lmilml - np.eye(N)]

    #rownetworks = [in_lmil - np.eye(N), in_lmimmil - np.eye(N), in_lml - np.eye(N),
       #            in_lmil2 - np.eye(N), in_lmimmil2 - np.eye(N),
         #          in_lmil3 - np.eye(N)] #0.91
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
        ...
        file_path_export = "/home/fatemeh/HAN-master/data/HCC/Export/Emb_lnc10.txt"
